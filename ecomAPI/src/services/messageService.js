        ...
        try {
            if (!data.roomId) {
             ...
             for(let i =0 ; i< message.length; i++){
                ...
                message[i].userData.image = toBinary(message[i].userData.image);
             }
            resolve({
                errCode: 0,
                data: message
        ...
        try {
            if (!userId) {
             ...
             for(let i =0 ; i< room.length; i++){
                ...
                room[i].userOneData.image = toBinary(room[i].userOneData.image);
                room[i].userTwoData = await db.User.findOne({where:{id:room[i].userTwo}})
                room[i].userTwoData.image = toBinary(room[i].userTwoData.image);
             }
            resolve({
                errCode: 0,
                data: room
