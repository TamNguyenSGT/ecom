...
import { toBinary } from "../utils/bufferSafe";
            ...
            for (let i = 0; i < res.rows.length; i++) {
                ...
                for (let j = 0; j < res.rows[i].productDetail.length; j++) {
                    ...
                    for (let k = 0; k < res.rows[i].productDetail[j].productImage.length; k++) {
                        res.rows[i].productDetail[j].productImage[k].image = toBinary(res.rows[i].productDetail[j].productImage[k].image)
                    }
                    if (!res.rows[i].productDetail[j].productImage || res.rows[i].productDetail[j].productImage.length === 0) {
                        res.rows[i].productDetail[j].productImage = [{ image: '' }]
                    }
                    // primary image for product card
                    if (!res.rows[i].image) {
                        res.rows[i].image = res.rows[i].productDetail?.[0]?.productImage?.[0]?.image || ''
                    }
                }
            }
            ...
            if (!id) {
                ...
                for (let i = 0; i < res.productDetail.length > 0; i++) {
                    ...
                    for (let j = 0; j < res.productDetail[i].productImage.length; j++) {
                        res.productDetail[i].productImage[j].image = toBinary(res.productDetail[i].productImage[j].image)
                    }
                    if (!res.productDetail[i].productImage || res.productDetail[i].productImage.length === 0) {
                        res.productDetail[i].productImage = [{ image: '' }]
                    }
                    ...
                    for (let i = 0; i < productdetail.rows.length; i++) {
                        ...
                        if (productdetail.rows[i].productImageData && productdetail.rows[i].productImageData.length > 0) {
                            for (let j = 0; j < productdetail.rows[i].productImageData.length > 0; j++) {
                                productdetail.rows[i].productImageData[j].image = toBinary(productdetail.rows[i].productImageData[j].image)
                            }
                        }
        ...
        try {
            if (!data.id || !data.limit || !data.offset) {
                ...
                if (productImage.rows && productImage.rows.length > 0) {
                    productImage.rows.map(item => item.image = toBinary(item.image))
                }
        ...
        try {
            if (!id) {
                ...
                if (productdetailImage) {
                    productdetailImage.image = toBinary(productdetailImage.image);
                }
                resolve({
                    errCode: 0,
            ...
            for (let i = 0; i < res.length; i++) {
                ...
                for (let j = 0; j < res[i].productDetail.length; j++) {
                    ...
                    for (let k = 0; k < res[i].productDetail[j].productImage.length; k++) {
                        res[i].productDetail[j].productImage[k].image = toBinary(res[i].productDetail[j].productImage[k].image)
                    }
                    if (!res[i].productDetail[j].productImage || res[i].productDetail[j].productImage.length === 0) {
                        res[i].productDetail[j].productImage = [{ image: '' }]
                    }
                    if (!res[i].image) {
                        res[i].image = res[i].productDetail?.[0]?.productImage?.[0]?.image || ''
                    }
                }
            }
                    ...
                    for (let g = 0; g < productArr.length; g++) {
                        ...
                        for (let j = 0; j < productArr[g].productDetail.length; j++) {
                            ...
                            for (let k = 0; k < productArr[g].productDetail[j].productImage.length; k++) {
                                productArr[g].productDetail[j].productImage[k].image = toBinary(productArr[g].productDetail[j].productImage[k].image)
                            }
                            if (!productArr[g].productDetail[j].productImage || productArr[g].productDetail[j].productImage.length === 0) {
                                productArr[g].productDetail[j].productImage = [{ image: '' }]
                            }
                            if (!productArr[g].image) {
                                productArr[g].image = productArr[g].productDetail?.[0]?.productImage?.[0]?.image || ''
                            }
                        }
                    }
                }
